...
const DirectionComponent: React.FC<DirectionProps> = ({
  start,
  end,
  selected,
}) => {
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      try {
        ...
        const traoptimalResponse = await axios.get(
          "http://localhost:8000/naver/direction",
          {
            params: {
        ...
        const trafastResponse = await axios.get(
          "http://localhost:8000/naver/direction",
          {
            params: {
              start: `${formattedStart.mapx},${formattedStart.mapy}`,
        ...
        const tracomfortResponse = await axios.get(
          "http://localhost:8000/naver/direction",
          {
            params: {
              start: `${formattedStart.mapx},${formattedStart.mapy}`,
  ...
  useEffect(() => {
    const loadScript = () => {
      return new Promise((resolve, reject) => {
        ...
        script.src =
          "https://openapi.map.naver.com/openapi/v3/maps.js?ncpClientId=sr9ox19ub6";
        script.async = true;
        script.onload = () => {
      ...
      .then(() => {
        if (traoptimalRoute) {
          ...
          const startLatLng = new window.naver.maps.LatLng(
            parseFloat(start.mapy.slice(0, 2) + "." + start.mapy.slice(2)),
            parseFloat(start.mapx.slice(0, 3) + "." + start.mapx.slice(3))
          );
          const endLatLng = new window.naver.maps.LatLng(
            parseFloat(end.mapy.slice(0, 2) + "." + end.mapy.slice(2)),
            parseFloat(end.mapx.slice(0, 3) + "." + end.mapx.slice(3))
          );
          ...
          const path = traoptimalRoute.path.map(
            ([x, y]: [number, number]) => new window.naver.maps.LatLng(y, x)
          );
    ...
    <DirectionComponentBox>
        ...
        <LocationDataBox>
          ...
          <LocationDataBoxBody>
            <p style={{ fontSize: "30px", fontWeight: "bold" }}>
              {Math.round(traoptimalRoute.summary.duration / 60 / 1000)}
            </p>
            <p style={{ fontSize: "22px", fontWeight: "bold" }}>분</p>
            <p style={{ fontSize: "20px", fontWeight: "bold" }}>
              &nbsp;⊸ {(traoptimalRoute.summary.distance / 1000).toFixed(1)}km
            </p>
          </LocationDataBoxBody>
          <LocationDataBoxFooter>
            <p style={{ fontSize: "15px", color: "gray" }}>
              택시비: {traoptimalRoute.summary.taxiFare}원 ⋯
            </p>
            <p style={{ fontSize: "15px", color: "gray" }}>
              &nbsp;주유비: {traoptimalRoute.summary.fuelPrice}원
            </p>
          </LocationDataBoxFooter>
        </LocationDataBox>
      )}
      {trafastRoute && (
        <LocationDataBox>
          ...
          <LocationDataBoxBody>
            <p style={{ fontSize: "30px", fontWeight: "bold" }}>
              {Math.round(trafastRoute.summary.duration / 60 / 1000)}
            </p>
            <p style={{ fontSize: "22px", fontWeight: "bold" }}>분</p>
            <p style={{ fontSize: "20px", fontWeight: "bold" }}>
              &nbsp;⊸ {(trafastRoute.summary.distance / 1000).toFixed(1)}km
            </p>
          </LocationDataBoxBody>
          <LocationDataBoxFooter>
            <p style={{ fontSize: "15px", color: "gray" }}>
              택시비: {trafastRoute.summary.taxiFare}원 ⋯
            </p>
            <p style={{ fontSize: "15px", color: "gray" }}>
              &nbsp;주유비: {trafastRoute.summary.fuelPrice}원
            </p>
          </LocationDataBoxFooter>
        </LocationDataBox>
      )}
      {tracomfortRoute && (
        <LocationDataBox>
          ...
          <LocationDataBoxBody>
            <p style={{ fontSize: "30px", fontWeight: "bold" }}>
              {Math.round(tracomfortRoute.summary.duration / 60 / 1000)}
            </p>
            <p style={{ fontSize: "22px", fontWeight: "bold" }}>분</p>
            <p style={{ fontSize: "20px", fontWeight: "bold" }}>
              &nbsp;⊸ {(tracomfortRoute.summary.distance / 1000).toFixed(1)}km
            </p>
          </LocationDataBoxBody>
          <LocationDataBoxFooter>
            <p style={{ fontSize: "15px", color: "gray" }}>
              택시비: {tracomfortRoute.summary.taxiFare}원 ⋯
            </p>
            <p style={{ fontSize: "15px", color: "gray" }}>
              &nbsp;주유비: {tracomfortRoute.summary.fuelPrice}원
            </p>
          </LocationDataBoxFooter>
        </LocationDataBox>
      )}
