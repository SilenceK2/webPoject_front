...
import {
  ...
  faTrain,
  faBus,
} from "@fortawesome/free-solid-svg-icons"; // 추가된 부분
...
const DirectionComponentPublic: React.FC<DirectionComponentPublicProps> = ({
  start,
  end,
  selected,
}) => {
      ...
      if (selected === "publicTransportation") {
        try {
          ...
          const response = await axios.get(
            "http://localhost:8000/odsay/route",
            {
              params: {
                SX,
                SY,
                ...
                <DirectionComponentPublicBox>
                  ...
                  <DirectionComponentPublicBoxBox>
                    <p style={{ fontSize: "30px", fontWeight: "bold" }}>
                      {Math.round(path.info.totalTime)}
                    </p>
                  ...
                  <DirectionComponentBar>
                    {path.subPath
                      .filter(
                        (subPath) =>
                          subPath.distance > 0 && subPath.sectionTime > 0
                      )
                      .map((subPath, subIndex) => {
                        const subPathWidth = Math.max(
                          (subPath.sectionTime / path.info.totalTime) * 100,
                          MIN_WIDTH
                        );
