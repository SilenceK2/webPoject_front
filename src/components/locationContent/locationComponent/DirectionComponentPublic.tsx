...
const DirectionComponentPublic: React.FC<DirectionComponentPublicProps> = ({ start, end, selected }) => {
  ...
  const MIN_WIDTH = 20; // 최소 폭 설정

  return (
    <>
      {route && (
        <DirectionComponentPublicSection>
          {route.path.slice(0, 5).map((path, index) => ( // 경로를 5개로 제한
            <div key={index}>
              <DirectionComponentPublicBox>
                <DirectionComponentPublicBoxHeader>
                ...
                <DirectionComponentBar>
                  {path.subPath.filter(subPath => subPath.distance > 0 && subPath.sectionTime > 0).map((subPath, subIndex) => {
                    const subPathWidth = Math.max((subPath.sectionTime / path.info.totalTime) * 100, MIN_WIDTH);

                    return (
                      <div key={subIndex} style={{ position: 'relative', display: 'inline-block', width: `${subPathWidth}%`, textAlign: 'center' }}>
                        ...
                        ></DirectionComponentPublicBar>
                        <div style={{ position: 'absolute', top: '20px', width: '100%', fontSize: '11px', fontWeight: 'bold' }}>
                        ...
                        {subPath.distance !== 0 && (
                          <div style={{ position: 'absolute', top: '0px', width: '100%', fontSize: '9px' }}>
